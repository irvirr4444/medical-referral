...
/** Split `total` into `count` positive integers that sum exactly to `total`. */
function splitTotal(total: number, count: number, seedKey: string): number[] {
  ...
  const weights = Array.from({ length: safeCount }, () => 0.55 + rand() * 0.9)
  const weightSum = weights.reduce((sum, weight) => sum + weight, 0)
  const exact = weights.map((weight) => (safeTotal * weight) / weightSum)
  const floors = exact.map((value) => ({
    base: Math.floor(value),
    frac: value - Math.floor(value),
  }))
  let remaining = safeTotal - floors.reduce((sum, item) => sum + item.base, 0)
  floors
    .map((item, index) => ({ ...item, index }))
    .sort((a, b) => b.frac - a.frac)
    .forEach((item) => {
      if (remaining <= 0) return
      floors[item.index].base += 1
      remaining -= 1
    })
  return floors.map((item) => item.base)
}

const WEEKDAY_LABELS = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'] as const

/**
 * Deterministic daily trend series for the Impact board chart.
 * Totals match the provided `total` exactly; shape is stable per scope/period.
 */
export function buildImpactTrendSeries(
  scope: ImpactScope,
  periodId: ImpactPeriodId | 'custom',
  total: number,
  dayCount?: number,
): ImpactTrendPoint[] {
  const seedKey = `${scope}:${periodId}:${Math.round(total)}:${dayCount ?? ''}`

  if (periodId === 'today') {
    const values = splitTotal(total, 5, seedKey)
  ...
  if (periodId === 'month') {
    ...
    const values = splitTotal(total, labels.length, seedKey)
  ...
  const days = Math.min(14, Math.max(3, dayCount ?? 7))
  const values = splitTotal(total, days, seedKey)
