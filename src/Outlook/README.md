...
- `review_mail.py`: review-email sending and unique reply-body retrieval.
...
Extract the newest referral and email its review summary without changing Monday:
...
The CLI checks one message by default, uses the canonical Anthropic Files API
extractor, performs read-only Monday duplicate and agency lookups, stores
timestamped audit artifacts under `tmp/inbox-runs/`, and prints progress. The
approval command writes Monday first and then records a DRK handoff. DRK automatic
patient creation is still disabled. Run the following for all options:
